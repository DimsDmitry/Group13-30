...
def save_answers():
    # получает из формы, проверяет правильность ответа, записывает итоги в словарь session
    answer = request.form.get('ans_text')
    quest_id = request.form.get('q_id')
    # записываем вопрос
    session['last_question'] = quest_id
    session['total'] += 1
    # проверяем, совпадает ли id ответа с правильным
    if check_answer(quest_id, answer):
        session['answers'] += 1


def question_form(question):
    # получает из БД строку с вопросом, возвращает html с формой
    # question - результат работы get_question_after,
    # поля:
    # [0] - номер вопроса в викторине
    # [1] - текст вопроса
    # [2] - правильный ответ
    # [3], [4], [5] - неправильные ответы
...
def test():
    ...
    if not ('quiz' in session) or int(session['quiz']) < 0:
        ...
        # если нам приходят данные, надо их прочесть и обновить информацию
